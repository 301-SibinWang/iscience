...
#     print(f"[INFO] No 'Rater' column detected, skip ICC filtering. Features kept: {icc_filtered_features.shape[1]-1}")
# else:
...
#     print(f"[INFO] ICC filtering done: {len(good_feats)} features kept (≥ {ICC_THRESHOLD}), saved -> icc_values.csv")
...
train = merge_data(train_df, features)
valid = merge_data(valid_df, features)
test  = merge_data(test_df, features)

img_features = [c for c in features.columns if c not in ['PatientCode']]
...
lasso = LassoCV(cv=10, alphas=np.logspace(-4,0,50), random_state=RANDOM_SEED).fit(X_train_img_filtered, y_train)
coef_mask = lasso.coef_ != 0
final_img_features = selected_img_features[coef_mask]

pd.DataFrame(final_img_features, columns=["SelectedFeature"]).to_csv(
    "lasso_selected_features.csv", index=False, encoding="utf-8-sig"
)
...
print(f"[INFO] LASSO (10-fold CV): {len(final_img_features)} features kept -> lasso_selected_features.csv")
...
lambda_vals = lasso.alphas_
lambda_min_idx = np.argmin(mse_mean)
lambda_min = lambda_vals[lambda_min_idx]


mse_min_plus_std = mse_mean[lambda_min_idx] + mse_std[lambda_min_idx]
lambda_1se_idx = np.where(mse_mean <= mse_min_plus_std)[0][-1]
lambda_1se = lambda_vals[lambda_1se_idx]

plt.figure(figsize=(7,5))
plt.errorbar(np.log10(lambda_vals), mse_mean, yerr=mse_std, fmt='-o', ecolor='lightgray', capsize=3, lw=2)
plt.axvline(np.log10(lambda_min), color='red', linestyle='--', label=f"lambda.min = {lambda_min:.4f}")
plt.axvline(np.log10(lambda_1se), color='blue', linestyle='--', label=f"lambda.1se = {lambda_1se:.4f}")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("lasso_cv_error_curve.png", dpi=300)
plt.show()

alphas_path, coefs_path, _ = lasso_path(X_train_img_filtered, y_train, alphas=np.logspace(-4,0,50))
...
plt.grid(True)
plt.tight_layout()
plt.savefig("lasso_coef_path.png", dpi=300)
plt.show()


coef_values = lasso.coef_[coef_mask]
feat_names = final_img_features
sorted_idx = np.argsort(np.abs(coef_values))[::-1]

plt.figure(figsize=(8,6))
plt.barh(feat_names[sorted_idx], np.abs(coef_values)[sorted_idx])
plt.xlabel("Coefficient Magnitude")
plt.title("Selected Feature Importance (LASSO)")
...
sns.heatmap(X_train_img_filtered[:, coef_mask], cmap="coolwarm", cbar=True)
